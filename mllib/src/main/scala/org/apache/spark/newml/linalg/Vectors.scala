...
import org.apache.spark.newml.util.NumericParser
...
sealed trait Vector extends Serializable {
   ...
   */
  private[newml] def toBreeze: BV[Double]
...
private[spark] class VectorUDT extends UserDefinedType[Vector] {
  ...
  override def pyUDT: String = "pyspark.newml.linalg.VectorUDT"

  override def userClass: Class[Vector] = classOf[Vector]
}

/**
 * Factory methods for [[org.apache.spark.newml.linalg.Vector]].
...
object Vectors {
   ...
   * an [[org.apache.spark.newml.linalg.Vector]].
   */
  def parse(s: String): Vector = {
    parseNumeric(NumericParser.parse(s))
  }

  private[newml] def parseNumeric(any: Any): Vector = {
    any match {
   ...
   */
  private[newml] def fromBreeze(breezeVector: BV[Double]): Vector = {
    breezeVector match {
      case v: BDV[Double] =>
...
class DenseVector(val values: Array[Double]) extends Vector {
  ...
  private[newml] override def toBreeze: BV[Double] = new BDV[Double](values)
...
class SparseVector(
  ...
  private[newml] override def toBreeze: BV[Double] = new BSV[Double](indices, values, size)
