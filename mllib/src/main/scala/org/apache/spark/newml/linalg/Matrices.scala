...
package org.apache.spark.newml.linalg
...
sealed trait Matrix extends Serializable {
  ...
  private[newml] def toBreeze: BM[Double]

  /** Gets the (i, j)-th element. */
  private[newml] def apply(i: Int, j: Int): Double

  /** Return the index for the (i, j)-th element in the backing array. */
  private[newml] def index(i: Int, j: Int): Int

  /** Update element at (i, j) */
  private[newml] def update(i: Int, j: Int, v: Double): Unit
  ...
  private[newml] def transposeMultiply(y: DenseMatrix): DenseMatrix = {
  ...
  private[newml] def transposeMultiply(y: DenseVector): DenseVector = {
...
class DenseMatrix(val numRows: Int, val numCols: Int, val values: Array[Double]) extends Matrix {
  ...
  private[newml] def toBreeze: BM[Double] = new BDM[Double](numRows, numCols, values)

  private[newml] def apply(i: Int): Double = values(i)

  private[newml] def apply(i: Int, j: Int): Double = values(index(i, j))

  private[newml] def index(i: Int, j: Int): Int = i + numRows * j
...
/**
 * Factory methods for [[org.apache.spark.newml.linalg.Matrix]].
 */
object Matrices {
   ...
   */
  private[newml] def fromBreeze(breeze: BM[Double]): Matrix = {
    breeze match {
      case dm: BDM[Double] =>
