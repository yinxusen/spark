...
private[spark] class ProxyRDDPartition (
    val index: Int,
 ...
 */
class ProxyRDD[K, V, C](
    var dependency: ShuffleDependency[K, V, C],
    var partitionStartIndices: Array[Int])
  ...
  def setPartitionStartIndices(parIndices: Array[Int]): this.type = {
    partitionStartIndices = parIndices
    this
  }

  override def getDependencies: Seq[Dependency[_, _]] = List(dependency)
  ...
  override def getPartitions: Array[Partition] = {
    ...
    Array.tabulate[Partition](partitionStartIndices.length) { i =>
      ...
      new ProxyRDDPartition(i, startIndex, endIndex)
    }
  }

  override def compute(p: Partition, context: TaskContext): Iterator[(K, C)] = {
    val part = p.asInstanceOf[ProxyRDDPartition]
